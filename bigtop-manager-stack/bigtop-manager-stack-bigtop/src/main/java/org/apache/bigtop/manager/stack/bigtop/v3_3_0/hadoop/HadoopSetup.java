...
public class HadoopSetup {
    ...
    private static boolean checkAllJournalNodesPortReachable(HadoopParams hadoopParams) throws InterruptedException {
        ...
        while (retryCount < maxRetry) {
            ...
            for (String host : journalNodeList) {
                ...
                try {
                    ...
                    allReachable = false;
                    log.warn("JournalNode [{}:{}] is NOT reachable (retry {}/{}). Error: {}",
                            host, port, retryCount + 1, maxRetry, e.getMessage());
                } finally {
        ...
        log.error("Failed to reach all JournalNodes after {} retries. JournalNode list: {}",
                maxRetry, journalNodeList);
        return false;
    }
