...
//        String cmd = MessageFormat.format("{0}/bin/solr start -cloud -force", solrParams.serviceHome());
        String cmd = MessageFormat.format("{0}/bin/solr start -cloud -noprompt -s {1} -Dsolr.default.confdir={1} -z {2}", solrParams.serviceHome(),solrParams.confDir(),solrParams.ZK_HOST());
